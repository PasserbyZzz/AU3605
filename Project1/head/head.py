...
def segment_image_by_thresholds(
    ...
    for i in range(len(bounds) - 1):
        ...
        # 第一个分段 (背景) 特殊处理，包含 0
        if i == 0:
            mask = ~((image_gray >= lower_bound) & (image_gray <= upper_bound-30))
        # 其他分段 (不包含下界，包含上界)
        else:
            mask = (image_gray > lower_bound-20) & (image_gray <= upper_bound)
...
def main():
    """
    脚本主入口：运行 'head' (CT) 图像的复现。
    """
    
    # --- 用户配置区 ---
    IMAGE_FILES = {
        'head': Path("head/head.bmp"),     
    }

    # 论文中没有给出CT图像的 *确切* 阈值
    # 但描述了聚类/分段的数量
    CLUSTER_CONFIG = {
        # 图22(下) "two obvious clusters" -> 2个阈值, 3个分段
        'head': {
            'n_clusters': 2,
            'segment_names': ['(a) 背景', '(b) 软体组织', '(c) 骨头']
        }
    }
    # --- 结束配置区 ---
    ...
    print("开始复现CT图像 'head.bmp' 的多阈值分割分析 (Wang & Bai, 2003)")
    ...
    if all_results:
        ...
        print("\n未处理任何图像。请检查 IMAGE_FILES 字典中的 'head' 文件路径。")
